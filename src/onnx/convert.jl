...
"""
function convert_model(model::Proto.OperatorSetIdProto)
    a = Dict{Symbol, Any}()
...
"""
function convert_model(model::Proto.StringStringEntryProto)
    a = Dict{Symbol, Any}()
    fields = [:key, :value]
    for ele in fields
        a[ele] = getfield(model, ele)
...
function write_julia_file(model_file)
    f = readproto(open(model_file), Proto.ModelProto())
    data = code(convert(f).graph)
    touch("model.jl")
